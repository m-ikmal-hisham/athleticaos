...
export const matchLineupService = {
    async getLineup(matchId: string, teamId: string): Promise<MatchLineupEntry[]> {
        const response = await axios.get<MatchLineupEntry[]>(`/api/v1/matches/${matchId}/lineup`, {
            params: { teamId }
        });
    ...
    async updateLineup(matchId: string, teamId: string, entries: MatchLineupEntry[]): Promise<MatchLineupEntry[]> {
        const response = await axios.put<MatchLineupEntry[]>(`/api/v1/matches/${matchId}/lineup`, {
            teamId,
            entries
        });
        return response.data;
    ...
    async getHints(matchId: string): Promise<any> {
        const response = await axios.get(`/api/v1/matches/${matchId}/lineup/hints`);
        return response.data;
    }
};
