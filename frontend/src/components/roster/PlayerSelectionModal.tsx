...
export function PlayerSelectionModal({
    ...
    const handleSelectAll = () => {
        const allPlayerIds = new Set(filteredPlayers.map(p => p.playerId));
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md max-h-[600px] flex flex-col border border-slate-200 dark:border-slate-800">
                {/* Header */}
                    ...
                    {filteredPlayers.length > 0 && (
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm text-slate-500">
                                {selectedPlayerIds.size} of {filteredPlayers.length} selected
                            </span>
                        </div>
                    )}
                </div>
