...
public interface TournamentPlayerRepository extends JpaRepository<TournamentPlayer, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT tp.player.id, COUNT(DISTINCT tp.tournament.id) FROM TournamentPlayer tp WHERE tp.player.id IN :playerIds AND tp.isActive = true AND tp.tournament.deleted = false AND tp.player.deleted = false GROUP BY tp.player.id")
    List<Object[]> countActiveTournamentsGroupedByPlayerIds(@org.springframework.data.repository.query.Param("playerIds") java.util.Collection<UUID> playerIds);
}
