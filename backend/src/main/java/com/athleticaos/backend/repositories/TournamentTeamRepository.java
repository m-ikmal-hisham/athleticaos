...
public interface TournamentTeamRepository extends JpaRepository<TournamentTeam, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT tt FROM TournamentTeam tt JOIN FETCH tt.team t LEFT JOIN FETCH t.organisation LEFT JOIN FETCH tt.category WHERE tt.tournament.id = :tournamentId AND tt.isActive = true AND tt.deleted = false")
