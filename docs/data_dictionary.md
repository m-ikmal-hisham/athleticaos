...
| registration_no | VARCHAR | 20 | No | Platform registration number (`AOS-000123`). Generated by the database, immutable, never reused, not derived from personal data. |
| record_verification_status | VARCHAR | 32 | No | `VERIFIED` / `UNVERIFIED`. Administrator attestation that the record was checked; **no identification number is stored or compared**. |
| record_verified_at | TIMESTAMP | - | Yes | When the record was verified. |
| record_verified_by | UUID | - | Yes | Administrator who verified it (FK to users). |
| record_verified_by_name | VARCHAR | 255 | Yes | Name captured at verification time. |
| record_verification_method | VARCHAR | 32 | Yes | `PRE_REGISTRATION_RECORD` or `DOCUMENT_SIGHTED`. |
